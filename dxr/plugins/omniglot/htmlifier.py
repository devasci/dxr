...
""" Omniglot - Speaking all commonly-used version control systems.
    ...
    Omniglot first scans the project directory looking for the hallmarks of a
    VCS (such as the .hg or .git directory). It also looks for these in parent
    directories in case DXR is only parsing a fraction of the repository. Once
    this information is found, it attempts to extract upstream information about
    the repository. From this information, it builds the necessary information
    to reproduce the links.
    ...
    - check if the mercurial paths are specific to Mozilla's customization or
      not.
"""

# Global variables
tree = None
source_repositories = dict()

class VCS(object):
    ''' A class representing an abstract notion of a version-control system.
        In general, all path arguments to query methods should be normalized to
        be relative to the root directory of the VCS. '''
    ...
    def generate_diff(self, path):
        ''' Return a URL for a page that shows the last change made to this
            file. '''
        raise NotImplemented()
...
class Mercurial(VCS):
    def __init__(self, root):
        ...
        self.untracked_files = [line.split()[1] for line in
...
class Git(VCS):
    def __init__(self, root):
        VCS.__init__(self, root)
        self.untracked_files = [line for line in
...
def load(tree_, conn):
    ...
    # directory first.
    lookup_order = list(source_repositories.keys())
    lookup_order.sort(lambda x, y: len(y) - len(x))
